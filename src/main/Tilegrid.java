...
//	 int Game.COLS = 25;
	...
	/*
	 * 0-15 = floor 16-31 = wall -1 = delete
	 */
	Game game;
	BufferedImage image;
	...
	public void reset(int kind) {
		this.grid = Utils.fill2DI(Game.COLS, Game.ROWS, kind);

		System.out.printf("Reset tilegrid data to kind %d\n", kind);

	}

	public BufferedImage[] initImages() throws IOException {
		try {
			this.placeholder = ImageIO.read(getClass().getResource("/images/goldbug.png")); // Image in
																							// resources/images/icon.png
		} catch (IOException e) {
			e.printStackTrace();
		}
		BufferedImage[] items1 = new Imageutils(game).spriteSheetCutter("/images/tileFloor.png", 4, 4, Game.TILE_SIZE,
				Game.TILE_SIZE);

		BufferedImage[] items2 = new Imageutils(game).spriteSheetCutter("/images/tileWall1.png", 4, 4, Game.TILE_SIZE,
				Game.TILE_SIZE);
...
//				this.game.g.fillRect(x*tileSize, y*tileSize, tileSize-1, tileSize-1);
				this.game.g.drawImage(image, x * tileSize - game.cameraX, y * tileSize - game.cameraY, tileSize + 1,
						tileSize + 1, null);
